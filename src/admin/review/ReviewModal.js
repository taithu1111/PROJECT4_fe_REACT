// src/admin/review/ReviewModal.js
import React, { useState } from 'react';

const ReviewModal = ({ review, onClose, onUpdate }) => {
    const [reviewText, setReviewText] = useState(review.review);
    const [loading, setLoading] = useState(false);

    const handleSubmit = async () => {
        if (reviewText.trim() === review.review.trim()) {
            alert('Nội dung bình luận chưa thay đổi');
            return;
        }
        ...
        setLoading(true);
        try {
            await onUpdate(review.id, reviewText);
            onClose();
        } catch (error) {
            ...
            <div className="bg-white rounded-lg p-6 w-full max-w-2xl">
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-900">{review.userEmail}</p>
                    </div>
                ...
                <div className="flex gap-3 mt-6">
                    ...
                    <button
                        ...
                        disabled={loading || reviewText.trim() === review.review.trim()}
